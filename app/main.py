...
async def get_barplot_data(request: BarPlotRequest):
    ...
    try:
        ...
        # Find annotation variable (discrete or string, optional)
        annot_var = None
        ...
        if color_var:
            ...
            for i in indices:
                c_val = color_col[i]
                if np.isnan(c_val):
                    colors.append([128, 128, 128])  # Gray for missing
                else:
                    colors.append(color_map.get(int(c_val), [128, 128, 128]))
        else:
            # Same color for all bars (gray)
            colors = [[128, 128, 128]] * len(data)
        
        # Calculate group separators (indices where group changes)
        group_separators = []
        if group_labels:
            prev_label = None
...
class HeatMapRequest(BaseModel):
    ...
    data_path: str
    color_scheme: str = "Blue-Green-Yellow"  # Color palette name
    threshold_low: float = 0.0  # Range start (0-1)
    threshold_high: float = 1.0  # Range end (0-1)
    merge_kmeans: bool = False  # Merge rows by k-means
    merge_kmeans_k: int = 50  # Number of clusters
    clustering_rows: str = "None"  # "None", "Clustering", "Clustering with optimal ordering"
    clustering_cols: str = "None"  # "None", "Clustering", "Clustering with optimal ordering"
    split_by_rows: Optional[str] = None  # Discrete variable to split rows
    split_by_cols: Optional[str] = None  # Discrete variable to split columns
    show_legend: bool = True
    show_averages: bool = True  # Show stripes with averages
    row_annotation_text: Optional[str] = None  # Variable for row annotation text
    row_annotation_color: Optional[str] = None  # Variable for row annotation color
    col_annotation_position: str = "Top"  # "Top", "Bottom", "Both"
    col_annotation_color: Optional[str] = None  # Variable for column annotation color
    keep_aspect_ratio: bool = False
    selected_indices: Optional[List[int]] = None  # Selected data indices


@api_v1.post("/visualize/heatmap", tags=["Visualize"])
async def get_heatmap_data(request: HeatMapRequest):
    ...
    try:
        ...
        # Get discrete variables for grouping/coloring
        ...
        # Build data matrix (only continuous variables)
        ...
        # Calculate data range before normalization
        ...
        if request.clustering_rows != "None" and len(norm_matrix) > 1:
            try:
                ...
                if len(norm_matrix) > 2:
                    ...
                    # Generate dendrogram data for frontend
        ...
        # Calculate column averages for stripe display
        col_averages = np.mean(norm_matrix, axis=0).tolist() if request.show_averages else None
        
        # Split by discrete variable (rows)
        split_groups = None
        if request.split_by_rows:
            ...
            if split_var:
                ...
                for val_idx, val_name in enumerate(split_var.values):
                    indices = np.where(split_col == val_idx)[0].tolist()
                    split_groups.append({
                        "name": val_name,
                        "indices": indices
                    })
        ...
        if request.row_annotation_text:
            ...
            if annot_var:
                if cluster_data:
                    # For clustered data, use cluster IDs as annotations
        ...
        if request.row_annotation_color:
            ...
            if color_var:
                ...
                for i in row_order:
                    if cluster_data:
                        # For clustered data, use most common color in cluster
                        row_annotation_colors.append([128, 128, 128])  # Gray for clusters
                    else:
