...
@router.get(
    "/logistic-regression/options", response_model=LogisticRegressionOptionsResponse
)
...
@router.post(
    "/logistic-regression/train", response_model=LogisticRegressionTrainResponse
)
async def train_logistic_regression(
    request: LogisticRegressionTrainRequest, x_session_id: Optional[str] = Header(None)
):
    ...
    try:
        ...
        logger.info(
            f"Loading Logistic Regression data from: {request.data_path} (session: {x_session_id})"
        )
            ...
            if hasattr(model, "coefficients") and hasattr(model, "domain"):
                ...
                for i, attr in enumerate(model.domain.attributes):
                    coefs.append(
                        {
                            "feature": attr.name,
                            "coefficient": float(model.coefficients[0][i])
                            if len(model.coefficients.shape) > 1
                            else float(model.coefficients[i]),
                        }
                    )
                if hasattr(model, "intercept"):
                    coefs.insert(
                        ...
                        {
                            "feature": "intercept",
                            "coefficient": float(model.intercept[0])
                            if hasattr(model.intercept, "__len__")
                            else float(model.intercept),
                        },
                    )
                coefficients = coefs
...
async def get_logistic_regression_coefficients(
    ...
    try:
        ...
        if hasattr(model, "domain"):
            ...
            feature_names = [
                f"feature_{i}" for i in range(model_info.get("features", 0))
            ]
