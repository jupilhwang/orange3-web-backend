...
async def train_knn(
    request: KNNRequest, x_session_id: Optional[str] = Header(None)
) -> KNNResponse:
    """
    Train a kNN model.
    ...
    if not ORANGE_AVAILABLE:
        return KNNResponse(success=False, error="Orange3 not available")

    try:
        # Validate parameters
        if request.n_neighbors < 1 or request.n_neighbors > 100:
            raise HTTPException(
                status_code=400, detail="Number of neighbors must be between 1 and 100"
            )
        ...
        logger.info(
            f"Loading kNN data from: {request.data_path} (session: {x_session_id})"
        )
        ...
        if not data.domain.class_var:
            raise HTTPException(
                status_code=400, detail="Data must have a target variable for kNN"
            )
        ...
        traceback.print_exc()
        return KNNResponse(success=False, error=str(e))
...
async def get_knn_options():
    ...
    return {
        ...
        "n_neighbors": {"min": 1, "max": 100, "default": 5},
    }
