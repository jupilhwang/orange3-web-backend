...
def calculate_error(
    actual: float, predicted: float, error_type: str
) -> Optional[float]:
...
def calculate_regression_metrics(
    actuals: np.ndarray, predictions: np.ndarray, model_name: str
) -> Dict:
    ...
    if np.any(nonzero_mask):
        mape = (
            np.mean(
                np.abs(
                    (actuals[nonzero_mask] - predictions[nonzero_mask])
...
class PredictRequest(BaseModel):
    ...
    regression_error_type: str = (
        "none"  # none, difference, absolute_difference, relative, absolute_relative
    )
...
async def make_predictions(
    request: PredictRequest, x_session_id: Optional[str] = Header(None)
) -> PredictResponse:
    """
    ...
    if not ORANGE_AVAILABLE:
        return PredictResponse(success=False, error="Orange3 not available")
    ...
    try:
        ...
        logger.info(
            f"Loading Predictions data from: {request.data_path} (session: {x_session_id})"
        )
        ...
        if len(data) == 0:
            return PredictResponse(success=False, error="No data to predict")
        ...
        for model_id in request.model_ids:
            if model_id not in all_models:
                return PredictResponse(
                    success=False, error=f"Model not found: {model_id}"
                )
        ...
        if not models:
            return PredictResponse(success=False, error="No models provided")
        ...
        if target_var:
            columns.append(
                {"id": target_var.name, "name": target_var.name, "type": "target"}
            )
        ...
        for i, (model, model_name) in enumerate(zip(models, model_names)):
            columns.append(
                {"id": f"pred_{i}", "name": model_name, "type": "prediction"}
            )

            # Add error column for regression
            if is_regression and request.regression_error_type != "none":
                columns.append({"id": f"error_{i}", "name": "error", "type": "error"})
        ...
        for row_idx in range(len(data)):
            ...
            if target_var:
                ...
                if target_var.is_discrete:
                    row_data[target_var.name] = (
                        str(target_val) if target_val is not None else None
                    )
                else:
            ...
            for i, model in enumerate(models):
                try:
                    ...
                    if target_var and target_var.is_discrete:
                        ...
                        if request.show_probabilities:
                            try:
                                ...
                                for j, class_val in enumerate(target_var.values):
                                    row_data[f"prob_{i}_{class_val}"] = round(
                                        float(probs[j]), 4
                                    )
                            except:
                                for class_val in target_var.values:
                        ...
                        if (
                            is_regression
                            and request.regression_error_type != "none"
                            and actual_val is not None
                        ):
                            error_val = calculate_error(
                                actual_val, pred_val, request.regression_error_type
                            )
