    ...
    try:
        ...
        if request.use_fixed:
            ...
            if request.cluster_range_from < 2 or request.cluster_range_to > 100:
                raise HTTPException(
                    status_code=400,
                    detail="Cluster range must be between 2 and 100"
                )
            if request.cluster_range_from >= request.cluster_range_to:
                raise HTTPException(
                    status_code=400,
                    detail="Range 'from' must be less than 'to'"
                )
        ...
        if request.use_fixed:
            ...
            for k_test in range(request.cluster_range_from, request.cluster_range_to + 1):
                if k_test >= len(data):
                    break
        ...
        cluster_var = DiscreteVariable(
            "Cluster",
            values=[f"C{i+1}" for i in range(k)]
        )
