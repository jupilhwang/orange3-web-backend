...
async def train_linear_regression(
    request: LinearRegressionRequest, x_session_id: Optional[str] = Header(None)
    ...
    if not ORANGE_AVAILABLE:
        return LinearRegressionResponse(success=False, error="Orange3 not available")
    ...
    try:
        ...
        if request.alpha < 0:
            raise HTTPException(status_code=400, detail="Alpha must be non-negative")

        if request.l1_ratio < 0 or request.l1_ratio > 1:
            raise HTTPException(
                status_code=400, detail="L1 ratio must be between 0 and 1"
            )
        ...
        logger.info(
            f"Loading Linear Regression data from: {request.data_path} (session: {x_session_id})"
        )
        ...
        if request.regularization_type == "none":
            ...
            learner = RidgeRegressionLearner(
                alpha=request.alpha, fit_intercept=request.fit_intercept
            )
        ...
        try:
            if hasattr(model, "skl_model") and hasattr(model.skl_model, "coef_"):
                ...
                for name, coef in zip(feature_names, coefs):
                    coefficients.append({"feature": name, "coefficient": float(coef)})

                if request.fit_intercept:
                    coefficients.insert(
                        0, {"feature": "(intercept)", "coefficient": float(intercept)}
                    )
        ...
        traceback.print_exc()
        return LinearRegressionResponse(success=False, error=str(e))
...
async def predict_linear_regression(
    ...
    try:
        ...
        return {"success": True, "predictions": pred_list, "count": len(pred_list)}
...
async def get_linear_regression_options():
    ...
    return {
        ...
        "alpha": {"min": 0.0001, "max": 1000, "default": 0.0001},
        "l1_ratio": {"min": 0, "max": 1, "default": 0.5},
    }
...
async def get_linear_regression_coefficients(
    ...
    try:
        ...
        coef_values = [
            c["coefficient"] for c in coefficients if not c.get("is_intercept")
        ]
