...
async def train_tree(
    request: TreeTrainRequest, x_session_id: Optional[str] = Header(None)
):
    ...
    try:
        ...
        logger.info(
            f"Loading Tree data from: {request.data_path} (session: {x_session_id})"
        )
    ...
    try:
        ...
        def node_to_dict(node, depth=0):
            ...
            result = {
                "id": id(node),
                "depth": depth,
                "samples": int(node.n_node_samples)
                if hasattr(node, "n_node_samples")
                else 0,
            }
            ...
            if hasattr(node, "attr") and node.attr is not None:
                ...
                result["split_value"] = (
                    float(node.value) if hasattr(node, "value") else None
                )
                ...
                if model.domain.class_var.is_discrete:
                    ...
                    if hasattr(node, "majority"):
                        result["majority_class"] = str(
                            model.domain.class_var.values[int(node.majority)]
                        )
                else:
        ...
        if tree_data is None:
            return {"success": False, "error": "Could not extract tree structure"}
    ...
    def build_node(node_id, depth=0):
        ...
        if left != -1:
            ...
            if hasattr(model, "domain") and model.domain.class_var.is_discrete:
                ...
                result["mean_value"] = (
                    float(value[0][0]) if len(value.shape) > 1 else float(value[0])
                )
