...
class DataSessionManager:
    ...
    async def cleanup_expired(cls) -> int:
        ...
        async with cls._get_lock():
            ...
            for session_id in list(cls._sessions.keys()):
                ...
                expired_ids = [
                    data_id
                    for data_id, entry in session_data.items()
    ...
    async def get_stats(cls) -> dict:
        ...
        async with cls._get_lock():
            ...
            return {"sessions": len(cls._sessions), "total_items": total_items}
...
#
# MIGRATION STATUS: Partially migrated to DataSessionManager.
#
# What has been done:
#   - load_data() tries DataSessionManager first for "sampler/" and "kmeans/"
#     prefixes before falling back to module-level dicts (_sampler_results,
#     _kmeans_results) imported from their respective modules.
#
# What remains (FIXME: complete when per-session isolation is required):
#   Step 1: Make widgets/data_sampler.py and widgets/kmeans.py store results
#           via DataSessionManager.store() instead of their module-level dicts.
#   Step 2: Remove _load_kmeans_data() and _load_sampler_data() helpers below
#           once all callers pass a valid session_id to load_data().
#   Step 3: Remove the "Fallback to legacy storage" branch in load_data().
#
# Rationale for deferral: The module-level dicts are process-global and work
# correctly for single-tenant deployments. Full migration requires coordinating
# session_id propagation through every widget endpoint, which is a larger
