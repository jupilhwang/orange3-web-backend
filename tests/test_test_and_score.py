...
class TestTestAndScoreWidget:
    ...
    def test_evaluate_random_sampling(self):
        ...
        response = client.post(
            ...
            json={
                "data_path": "iris",
                "learner_configs": [{"type": "knn", "name": "kNN", "n_neighbors": 5}],
    ...
    def test_evaluate_leave_one_out(self):
        ...
        response = client.post(
            ...
            json={
                "data_path": "iris",
                "learner_configs": [{"type": "knn", "name": "kNN", "n_neighbors": 3}],
    ...
    def test_evaluate_no_learners(self):
        ...
        if not result.get("success"):
            assert "No learners" in result.get(
                "error", ""
            ) or "not available" in result.get("error", "")
...
@pytest.mark.skipif(not ORANGE_AVAILABLE, reason="Orange3 required")
class TestTestAndScoreRegression:
    """Regression tests that strictly assert evaluation succeeds."""

    def test_cross_validation_knn_succeeds(self):
        """KNNLearner must be importable and cross-validation must succeed."""
        response = client.post(
            "/api/v1/evaluate/test_and_score/evaluate",
            json={
                "data_path": "iris",
        ...
        assert result["scores"][0]["CA"] is not None

    def test_cross_validation_multiple_learners_succeeds(self):
        """Multiple learner types must all be importable."""
        response = client.post(
            "/api/v1/evaluate/test_and_score/evaluate",
            json={
                "data_path": "iris",
